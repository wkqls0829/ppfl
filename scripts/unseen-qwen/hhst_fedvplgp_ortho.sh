...
# Unseen Client Experiment: FedVPL-GP-Ortho Selector Training (Qwen 2)
# TID: 70003
# GPU: 6 (selector on GPU 6 per README)
...
echo "=========================================="
echo "Unseen Client Experiment: FedVPL-GP-Ortho Selector Training (Qwen 2)"
echo "=========================================="
echo "Task ID: ${tid}"
echo "Device: ${device}"
...
echo "Config: cfg/fedvpl-gp-ortho-unseen/hhst.yaml"
echo "Checkpoint: ${CHECKPOINT_DIR}/hhrl_choice_qwen2_fedvplgp_ortho_unseen_t${tid}.ckpt"
echo "Log file: outputs/${tid}.log"
echo "=========================================="
echo ""

nohup python -u federatedscope/main.py \
    --cfg cfg/fedvpl-gp-ortho-unseen/hhst.yaml \
    device ${device} \
    federate.client_num 20 \
    federate.sample_client_num 5 \
    data.root ${WORK_DIR}/data \
    federate.save_to ${CHECKPOINT_DIR}/hhrl_choice_qwen2_fedvplgp_ortho_unseen_t${tid}.ckpt \
    expname "fedvplgp_ortho_unseen_hhst_t${tid}" \
    > outputs/${tid}.log 2>&1 &
