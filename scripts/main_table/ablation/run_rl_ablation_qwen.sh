...
# Ablation Study RL Training Script for Qwen 2
# SLURM cluster execution script
# TID range: 63400-63432 (Qwen 2 ablation RL experiments)
...
RL_TID=$3  # RL Task ID (e.g., 63400)
SELECTOR_TID=$4  # Selector Task ID (e.g., 62400)
...
if [ -z "$METHOD" ] || [ -z "$CLIENT_COUNT" ] || [ -z "$RL_TID" ] || [ -z "$SELECTOR_TID" ]; then
    ...
    echo "  rl_tid: RL Task ID (e.g., 63400)"
    echo "  selector_tid: Selector Task ID (e.g., 62400)"
    exit 1
fi
