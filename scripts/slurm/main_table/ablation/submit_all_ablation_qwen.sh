...
# Submit all Qwen 2 ablation study experiments to SLURM
...
echo "=========================================="
echo "Submitting Qwen 2 Ablation Study Experiments"
echo "=========================================="
...
for method in "${METHODS[@]}"; do
    for client_count in "${CLIENT_COUNTS[@]}"; do
        ...
        sbatch --job-name="ablation_sel_${method}_n${client_count}_${tid}" \
               scripts/slurm/main_table/ablation/run_selector_ablation_qwen.sh \
...
echo "Please run submit_rl_ablation_qwen.sh after selector jobs complete."
