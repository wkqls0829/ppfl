...
# Ablation Study RL Training Script for Gemma-2B
# SLURM cluster execution script
# TID range: 63300-63332 (Gemma-2B ablation RL experiments)
...
RL_TID=$3  # RL Task ID (e.g., 63300)
SELECTOR_TID=$4  # Selector Task ID (e.g., 62300)
...
if [ -z "$METHOD" ] || [ -z "$CLIENT_COUNT" ] || [ -z "$RL_TID" ] || [ -z "$SELECTOR_TID" ]; then
    ...
    echo "  rl_tid: RL Task ID (e.g., 63300)"
    echo "  selector_tid: Selector Task ID (e.g., 62300)"
    exit 1
fi
...
case $METHOD in
    vplgp)
        ...
        CONFIG_BASE="cfg/vpl-gp-no-ortho/hrl.yaml"
        METHOD_NAME="vplgp"
        USE_SELECTOR=true
        ;;
    vplortho)
        TRAINER="llmdporewardtrainer"
        CONFIG_BASE="cfg/vpl-ortho/hrl.yaml"
...
SELECTOR_CKPT="$CHECKPOINT_DIR/final_hhrl_choice_${MODEL}_ablation_${METHOD_NAME}_t${SELECTOR_TID}.ckpt"
if [ ! -f "$SELECTOR_CKPT" ]; then
    # Try regular checkpoint
    SELECTOR_CKPT="$CHECKPOINT_DIR/hhrl_choice_${MODEL}_ablation_${METHOD_NAME}_t${SELECTOR_TID}.ckpt"
    if [ ! -f "$SELECTOR_CKPT" ]; then
        echo "ERROR: Selector checkpoint not found:"
        echo "  Tried: $CHECKPOINT_DIR/final_hhrl_choice_${MODEL}_ablation_${METHOD_NAME}_t${SELECTOR_TID}.ckpt"
        echo "  Tried: $CHECKPOINT_DIR/hhrl_choice_${MODEL}_ablation_${METHOD_NAME}_t${SELECTOR_TID}.ckpt"
        exit 1
    fi
fi
...
CONFIG_FILE="cfg/main_table/ablation/${MODEL}/${METHOD_NAME}/hrl_n${CLIENT_COUNT}_${RL_TID}.yaml"
...
config['federate']['save_to'] = "$CHECKPOINT_DIR/hhrl_rlhf_${MODEL}_ablation_${METHOD_NAME}_t${RL_TID}.ckpt"
...
# Update expname
config['expname'] = "${METHOD_NAME}_${MODEL}_n${CLIENT_COUNT}_rl_t${RL_TID}"
...
if [ "$USE_SELECTOR" == "true" ]; then
    SELECTOR_CFG="cfg/main_table/ablation/${MODEL}/${METHOD_NAME}/hhst_n${CLIENT_COUNT}_${SELECTOR_TID}.yaml"
...
# Run experiment
echo "Starting ablation RL training: $METHOD_NAME, $MODEL, N=$CLIENT_COUNT, RL_TID=$RL_TID"
...
echo "Ablation RL experiment completed: TID=$RL_TID"
