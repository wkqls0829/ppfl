...
# On local server (no SLURM), use biscuit conda env
...
if [ "$USE_SELECTOR" == "true" ]; then
    ...
    if [ -z "$SELECTOR_CKPT" ] || [ ! -f "$SELECTOR_CKPT" ]; then
        echo "ERROR: Selector checkpoint not found for $SELECTOR_TID in $CHECKPOINT_DIR or $WORK_DIR/checkpoints"
        exit 1
    fi
...
if "$USE_SELECTOR" == "true":
    ...
    config['llm']['vpl_gp_temperature'] = 5.0  # Gemma Phase 5 (match selector)
    
    # For FedVPA-GP
    if "$METHOD" == "fedvpagp":
        config['llm']['vpl_use_gp_prior'] = True
        config['llm']['vpl_kl_weight'] = 0.05  # Match selector (Phase 5)
