...
# Single GPU mode: GPU 1 (specified in config file)

tid=10000
# GPU is specified in config file (device: 1)
...
nohup python -u federatedscope/llm/rlhf/main.py \
    --cfg cfg/feddpo/hrl-10000.yaml \
    --selector-cfg-file cfg/feddpo/hrl-10000.yaml \
...
echo "Config: cfg/feddpo/hrl-10000.yaml (pure DPO, no VPL, no selector required)"
echo "Hyperparameters: batch_size=1, lr=0.0001, grad_accum_step=4, reward_coeff=0.1"
echo "GPU: 1 (specified in config file: device: 1)"
