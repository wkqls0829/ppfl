...
# HRL (hh-rlhf) version with DPO training
# FedDPO is pure DPO for federated learning (baseline)
# WITHOUT VPL, GP prior, or orthogonal loss
# Direct Preference Optimization: https://arxiv.org/abs/2305.18290
# Single GPU mode: GPU 6 (specified in config file)

tid=11024
# GPU is specified in config file (device: 6)
...
nohup python -u federatedscope/llm/rlhf/main.py \
    --cfg cfg/feddpo/hrl-11024.yaml \
    --selector-cfg-file cfg/feddpo/hrl-11024.yaml \
...
echo "Config: cfg/feddpo/hrl-11024.yaml (pure DPO, no VPL, no selector required)"
echo "Hyperparameters: batch_size=1, lr=0.0001, grad_accum_step=4, reward_coeff=0.1"
echo "GPU: 6 (specified in config file: device: 6)"
echo "Client num: 10 (hh-rlhf dataset split)"
