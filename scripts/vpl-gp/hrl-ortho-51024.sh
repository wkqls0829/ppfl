...
# Using 50024 selector checkpoint
# Single GPU mode: GPU 0 (specified in config file)

tid=51024
selector_tid=50024  # Selector checkpoint task ID (vpl-gp-ortho 50024)

# GPU is specified in config file (device: 3)
...
if [ -f "${SELECTOR_CKPT_FINAL}" ]; then
    ...
    echo "Please run vpl-gp/hhst-ortho-${selector_tid}.sh first to generate the selector checkpoint (tid=${selector_tid})."
    exit 1
fi
echo ""

nohup python -u federatedscope/llm/rlhf/main.py \
    --cfg cfg/vpl-gp/hrl-ortho-51024.yaml \
    --selector-cfg-file cfg/vpl-gp/hhst-ortho-${selector_tid}.yaml \
...
echo "Config: cfg/vpl-gp/hrl-ortho-51024.yaml (z-dependent generation ENABLED)"
echo "Selector checkpoint: ${selector_tid} (vpl-gp-ortho 50024)"
