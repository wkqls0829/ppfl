...
# RL training script for VPL-GP (Variational Preference Learning with Gumbel Softmax Prior) with 100 clients
...
# Selector checkpoint from vpl-gp/hhst_c100.sh (tid=50100)
SELECTOR_TID=50100
SELECTOR_CKPT="/hdd/hdd3/kjb/checkpoints/hhrl_choice_gemma_fedbiscuit_u3_vplgp_c100_${SELECTOR_TID}.ckpt"

echo "Starting HRL VPL-GP (task ID: ${tid}, 100 clients, GPU 3)"
echo "Selector checkpoint: ${SELECTOR_CKPT} (from vpl-gp/hhst_c100.sh, tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-gp/hrl.yaml"
echo "RLHF checkpoint: /hdd/hdd3/kjb/checkpoints/hhrl_rlhf_gemma_choice_vplgp_c100_${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please run vpl-gp/hhst_c100.sh first to generate the selector checkpoint."
    exit 1
fi
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    federate.save_to /hdd/hdd3/kjb/checkpoints/hhrl_rlhf_gemma_choice_vplgp_c100_${tid}.ckpt \
    expname "vplgp_hrl_c100_t${tid}" \
