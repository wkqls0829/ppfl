...
# Test run using 50000 selector checkpoint
# Single GPU mode: GPU 2 (specified in config file)

tid=51000
selector_tid=50000  # Selector checkpoint task ID (vpl-gp-ortho 50000 - test run)
...
if [ -f "${SELECTOR_CKPT_FINAL}" ]; then
    ...
    echo "Please run vpl-gp/hhst-ortho.sh first to generate the selector checkpoint (tid=${selector_tid})."
    exit 1
fi
echo ""

nohup python -u federatedscope/llm/rlhf/main.py \
    --cfg cfg/vpl-gp/hrl-ortho-51000.yaml \
...
echo "Config: cfg/vpl-gp/hrl-ortho-51000.yaml (z-dependent generation ENABLED)"
echo "Selector checkpoint: ${selector_tid} (vpl-gp-ortho 50000 - test run)"
