...
# Selector: 62412 (VPL+Ortho, N=100)
...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_qwen2_ablation_vplortho_n100_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_qwen2_ablation_vplortho_n100_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_qwen2_ablation_vplortho_n100_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation (Qwen 2): VPL + Ortho (no GP prior) - RL Training, N=100"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-ortho/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplortho_n100_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please ensure hhst_vplortho_qwen_n100.sh (tid=${SELECTOR_TID}) has completed successfully."
    exit 1
fi
...
temp_selector_config="/tmp/selector_config_vplortho_qwen_n100_${tid}_$$.yaml"
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplortho_n100_t${tid}.ckpt \
    expname "vpl_ortho_qwen2_hrl_n100_t${tid}" \
    > outputs/${tid}.log 2>&1 &
