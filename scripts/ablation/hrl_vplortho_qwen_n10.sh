...
# Ablation Study (Qwen 2): VPL + Ortho (orthogonal loss, no GP prior) - RL Training
# RLHF training script for local server execution
# Selector: 62410 (VPL+Ortho, N=10)
...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_qwen2_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_qwen2_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_qwen2_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation (Qwen 2): VPL + Ortho (no GP prior) - RL Training, N=10"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-ortho/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplortho_n10_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please ensure hhst_vplortho_qwen_n10.sh (tid=${SELECTOR_TID}) has completed successfully."
    exit 1
fi
...
temp_selector_config="/tmp/selector_config_vplortho_qwen_n10_${tid}_$$.yaml"
cp cfg/vpl-ortho/hhst.yaml "${temp_selector_config}"
# Update for Qwen 2
...
sed -i "s|client_num:.*|client_num: 10|" "${temp_selector_config}"

nohup python -u federatedscope/llm/rlhf/main.py \
    --selector-cfg-file "${temp_selector_config}" \
    --cfg cfg/vpl-ortho/hrl.yaml \
    device ${device} \
    data.root ${DATA_ROOT} \
    model.type "Qwen/Qwen2-0.5B@huggingface_llm" \
    train.optimizer.lr 0.00001 \
    llm.grad_accum_step 32 \
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplortho_n10_t${tid}.ckpt \
    expname "vpl_ortho_qwen2_hrl_n10_t${tid}" \
    > outputs/${tid}.log 2>&1 &
