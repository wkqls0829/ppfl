...
# Ablation Study: VPL + GP (Gumbel-Softmax Prior, no orthogonal loss) - RL Training
# RLHF training script for local server execution
# Based on vpl-gp/hrl_c100.sh structure

tid=63300
device=0  # Set GPU device number
SELECTOR_TID=62300  # Selector task ID from hhst_vplgp.sh
...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_gemma_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_gemma_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_gemma_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation: VPL + GP (no orthogonal loss) - RL Training"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (from hhst_vplgp.sh, tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-gp-no-ortho/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_ablation_vplgp_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please run scripts/ablation/hhst_vplgp.sh first to generate the selector checkpoint."
    exit 1
fi
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    device ${device} \
    data.root ${DATA_ROOT} \
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_ablation_vplgp_t${tid}.ckpt \
    expname "vplgp_no_ortho_hrl_t${tid}" \
    > outputs/${tid}.log 2>&1 &
