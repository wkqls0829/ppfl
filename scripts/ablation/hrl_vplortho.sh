...
# Ablation Study: VPL + Ortho (Orthogonal loss, no GP prior) - RL Training
# RLHF training script for local server execution
# Based on vpl-gp/hrl_c100.sh structure

tid=63310
device=1  # Set GPU device number
SELECTOR_TID=62310  # Selector task ID from hhst_vplortho.sh
...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_gemma_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_gemma_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        # Try 40_ checkpoint
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_gemma_ablation_vplortho_n10_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation: VPL + Ortho (no GP prior) - RL Training"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (from hhst_vplortho.sh, tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-ortho/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_ablation_vplortho_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please run scripts/ablation/hhst_vplortho.sh first to generate the selector checkpoint."
    exit 1
fi
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    device ${device} \
    data.root ${DATA_ROOT} \
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_ablation_vplortho_t${tid}.ckpt \
    expname "vpl_ortho_hrl_t${tid}" \
    > outputs/${tid}.log 2>&1 &
