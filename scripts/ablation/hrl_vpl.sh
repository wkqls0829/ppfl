...
# Ablation Study: VPL Baseline (no GP prior, no orthogonal loss) - RL Training
# RLHF training script for local server execution

tid=63320
device=4  # GPU 4 (same as selector)
SELECTOR_TID=39000  # Selector task ID from hhst_vpl_n10.sh
...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_gemma_vpl_n10_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_gemma_vpl_n10_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_gemma_vpl_n10_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation: VPL Baseline - RL Training"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (from hhst_vpl_n10.sh, tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_vpl_n10_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please run scripts/ablation/hhst_vpl_n10.sh first to generate the selector checkpoint."
    exit 1
fi
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    device ${device} \
    data.root ${DATA_ROOT} \
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_gemma_vpl_n10_t${tid}.ckpt \
    expname "vpl_hrl_n10_t${tid}" \
    > outputs/${tid}.log 2>&1 &
