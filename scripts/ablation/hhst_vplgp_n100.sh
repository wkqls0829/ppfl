...
# Ablation Study: VPL + GP (Gumbel-Softmax Prior, no orthogonal loss)
...
echo "Starting Ablation: VPL + GP (no orthogonal loss) - Selector Training"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Client count: 100"
echo "Config: cfg/vpl-gp-no-ortho/hhst.yaml"
echo "Checkpoint: ${CHECKPOINT_DIR}/hhrl_choice_gemma_ablation_vplgp_n100_t${tid}.ckpt"
...
nohup python -u federatedscope/main.py \
    ...
    federate.save_to ${CHECKPOINT_DIR}/hhrl_choice_gemma_ablation_vplgp_n100_t${tid}.ckpt \
    expname "vplgp_no_ortho_hhst_n100_t${tid}" \
    > outputs/${tid}.log 2>&1 &
