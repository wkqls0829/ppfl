...
# Ablation Study: VPL + Ortho (Orthogonal loss, no GP prior) - Qwen 2
...
echo "Starting Ablation: VPL + Ortho (no GP prior) - Qwen 2 - Selector Training"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Client count: 100"
echo "Config: cfg/vpl-ortho/hhst.yaml"
echo "Model: Qwen 2"
echo "Checkpoint: ${CHECKPOINT_DIR}/hhrl_choice_qwen2_ablation_vplortho_n100_t${tid}.ckpt"
...
nohup python -u federatedscope/main.py \
    ...
    federate.save_to ${CHECKPOINT_DIR}/hhrl_choice_qwen2_ablation_vplortho_n100_t${tid}.ckpt \
    expname "vpl_ortho_qwen2_hhst_n100_t${tid}" \
    > outputs/${tid}.log 2>&1 &
