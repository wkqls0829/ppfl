...
SELECTOR_CKPT="${CHECKPOINT_DIR}/final_hhrl_choice_qwen2_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
if [ ! -f "${SELECTOR_CKPT}" ]; then
    SELECTOR_CKPT="${CHECKPOINT_DIR}/hhrl_choice_qwen2_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
    if [ ! -f "${SELECTOR_CKPT}" ]; then
        SELECTOR_CKPT="${CHECKPOINT_DIR}/40_hhrl_choice_qwen2_ablation_vplgp_n10_t${SELECTOR_TID}.ckpt"
    fi
fi

echo "Starting Ablation (Qwen 2): VPL + GP (no orthogonal loss) - RL Training, N=10"
echo "Task ID: ${tid}"
echo "Device: ${device}"
echo "Selector checkpoint: ${SELECTOR_CKPT} (tid=${SELECTOR_TID})"
echo "RLHF config: cfg/vpl-gp-no-ortho/hrl.yaml"
echo "RLHF checkpoint: ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplgp_n10_t${tid}.ckpt"
...
if [ ! -f "${SELECTOR_CKPT}" ]; then
    ...
    echo "Please ensure hhst_vplgp_qwen_n10.sh (tid=${SELECTOR_TID}) has completed successfully."
    exit 1
fi
...
temp_selector_config="/tmp/selector_config_vplgp_qwen_n10_${tid}_$$.yaml"
...
nohup python -u federatedscope/llm/rlhf/main.py \
    ...
    federate.save_to ${CHECKPOINT_DIR}/hhrl_rlhf_qwen2_ablation_vplgp_n10_t${tid}.ckpt \
    expname "vplgp_no_ortho_qwen2_hrl_n10_t${tid}" \
    > outputs/${tid}.log 2>&1 &
