...
\textbf{Baseline VPL:} In the ablation baseline, we replace the federated mixture prior with a standard Gaussian prior $p(z) = \mathcal{N}_0$. The KL divergence has closed form:
\begin{equation}
...
\begin{equation}
    \mathbb{D}_{KL}(q_i \,\|\, p_{\text{mixture}}(z)) = \mathbb{E}_{z \sim q_i} \left[ \log q_i(z) - \log p_{\text{mixture}}(z) \right],
\end{equation}
...
\begin{align}
    ...
    &\quad + \beta \cdot \underbrace{\mathbb{D}_{KL}(q_i \,\|\, p_{\text{mixture}}(z))}_{\text{Regularization (Prior Matching)}} \nonumber \\
